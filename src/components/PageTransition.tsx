...
export default function PageTransition({ children }: PageTransitionProps) {
    ...
    const [isTransitioning, setIsTransitioning] = useState(true); // Start with animation
    const [animationStep, setAnimationStep] = useState<'split' | 'merge' | 'zoom' | 'complete'>('split');
    const [showContent, setShowContent] = useState(false); // Start with no content
    ...
    useEffect(() => {
        ...
        // Listen for custom transition events (for navigation)
                    ...
                    >
                        <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
                            ...
                            <motion.div
                                ...
                                animate={{
                                    x: animationStep === 'split' ? -2 : 0,
                                    opacity: 1,
                                    rotate: animationStep === 'split' ? -0.5 : 0,
                                    scale: animationStep === 'zoom' ? 3 : 1,
                                }}
                                transition={{
                                    duration: animationStep === 'split' ? 0.4 :
                                        animationStep === 'merge' ? 0.4 : 0.6,
                                    ease: "easeInOut"
                                }}
                                className="relative z-10"
                            >
                                <svg width="600" height="1071" viewBox="0 0 244 218" className="overflow-visible drop-shadow-2xl">
                                    <defs>
                                    ...
                                    <path d={pPart} fill="url(#leftGradient)" transform="translate(115,21)" />
                                </svg>
                            </motion.div>
                            ...
                            >
                                <svg width="600" height="1071" viewBox="0 0 244 218" className="overflow-visible drop-shadow-2xl">
                                    ...
                                    <path d={aPart} fill="url(#rightGradient)" transform="translate(127,27)" />
                                </svg>
                            </motion.div>
                        </div>
                    </motion.div>
